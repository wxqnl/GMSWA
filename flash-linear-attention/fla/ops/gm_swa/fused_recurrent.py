...
def fused_recurrent_gm_swa_fwd_kernel(
    ...
    i_d, i_hm, i_n = tl.program_id(0), tl.program_id(1), tl.program_id(2)
...
@triton.heuristics({
    ...
    'IS_VARLEN': lambda args: args['cu_seqlens'] is not None,
    'TRUNCATE_BACKWARD': lambda args: args['truncate_backward'],
})
@triton.autotune(
    configs=[
        triton.Config({'BD': BD}, num_warps=num_warps)
...
def fused_recurrent_gm_swa_bwd_kernel(
    ...
    IS_VARLEN: tl.constexpr,
    TRUNCATE_BACKWARD: tl.constexpr,
):
    i_d, i_hm, i_n = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    ...
    for i in range(T - 1, -1, -1):
        ...
        if TRUNCATE_BACKWARD:
            b_local_dh = b_do
            if USE_FINAL_STATE_GRADIENT and i == (T - 1):
                b_local_dh += b_dh
            b_dx = b_local_dh * (1.0 - b_g)
            b_dg = tl.sum(b_local_dh * (b_prev - b_x), axis=0)
            b_dh = tl.zeros([BD], dtype=tl.float32)
        else:
            b_dh += b_do
            b_dx = b_dh * (1.0 - b_g)
...
def fused_recurrent_gm_swa_fwd(
    ...
    def grid(meta):
        return (triton.cdiv(D, meta['BD']), HM, N)
...
def fused_recurrent_gm_swa_bwd(
    ...
    fused_recurrent_gm_swa_bwd_kernel[grid](
        ...
        dh0=dh0,
        cu_seqlens=cu_seqlens,
        truncate_backward=truncate_backward,
        T=T,
        HM=HM,
        D=D,
    )
...
class FusedRecurrentGMSWAFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.truncate_backward = truncate_backward
        return o, ht
    ...
    def backward(ctx, do, dht=None):
        ...
        dx, dg, dh0 = fused_recurrent_gm_swa_bwd(
            ...
            truncate_backward=ctx.truncate_backward,
        )
        return dx, dg, dh0, None, None, None
