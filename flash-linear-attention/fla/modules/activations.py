...
def swiglu_fwd(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    try:
        ...
        return _swiglu_fwd_torch(x, y)


def _swiglu_fwd_torch(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    x_f = x.float()
    y_f = y.float()
    return (x_f * torch.sigmoid(x_f) * y_f).to(dtype=x.dtype)


def _swiglu_fwdbwd_torch(
    x: torch.Tensor,
    y: torch.Tensor,
    g: torch.Tensor,
    ...
    s = torch.sigmoid(x_f)
    x_s = x_f * s
    dx = (g_f * s * (1.0 + x_f * (1.0 - s)) * y_f).to(dtype=x.dtype)
    dy = (g_f * x_s).to(dtype=y.dtype)
    if use_weight:
        z = (x_s * y_f).to(dtype=x.dtype)
        return dx, dy, z
    return dx, dy
...
def swiglu_fwdbwd(x: torch.Tensor, y: torch.Tensor, g: torch.Tensor, use_weight: bool = False):
    ...
    try:
        ...
        if x.is_cuda:
            torch.cuda.empty_cache()
        return _swiglu_fwdbwd_torch(x, y, g, use_weight=use_weight)
    if use_weight:
        return dx, dy, z
    return dx, dy
