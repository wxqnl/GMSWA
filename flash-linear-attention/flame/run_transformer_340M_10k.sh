set -euo pipefail
# Full-attention Transformer 340M CONTROL — same data/recipe/global-batch as GMSWA/SWA.
# Upper bound: does full attention recall NIAH where windowed models cannot?

VENV=/home/user01/Minko/GMSWA/.venv311
FLAME=/home/user01/Minko/GMSWA/flash-linear-attention/flame
RUN=$FLAME/saves/Transformer-340M-10k
...
"$VENV/bin/torchrun" \
  ...
  --rdzv_backend c10d --rdzv_endpoint "localhost:29523" \
  --local-ranks-filter 0 --role rank --tee 3 \
  --log-dir "$RUN/logs" \
  -m flame.train \
  --job.dump_folder "$RUN" \
  --model.config configs/transformer_340M_ctrl.json \
