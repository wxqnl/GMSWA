#!/usr/bin/bash
# 1B launch template for SWA-TransformerHybrid-1B-10k.
...
RUN=${RUN:-$FLAME/saves/SWA-TransformerHybrid-1B-10k}
...
"$VENV/bin/torchrun" \
  ...
  -m flame.train \
  --job.dump_folder "$RUN" \
  --model.config configs/swa_transformer_hybrid_1B.json \
