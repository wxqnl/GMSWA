#!/usr/bin/bash
# GM-SWA v2 340M — 10000 steps, warmup 1000, 8×GPU (FSDP), long-context varlen.
# Env: project .venv311 (py3.11 / torch2.8 / working flash_attn).
set -euo pipefail
...
"$VENV/bin/torchrun" \
  ...
  --rdzv_backend c10d --rdzv_endpoint "localhost:29521" \
  --local-ranks-filter 0 --role rank --tee 3 \
  --log-dir "$RUN/logs" \
  -m flame.train \
  --job.dump_folder "$RUN" \
  --model.config configs/gated_mem_swa_340M.json \
