...
cd "$FLAME"
LOG=$ROOT/eval_results/eval_v8drop15.log
RUN=$FLAME/saves/GMSWA-340M-v8drop15-10k
CFG=configs/gated_mem_swa_v8drop15_340M.json
...
CUDA_VISIBLE_DEVICES=0 $PY "$ROOT/scripts/analysis/ppl_vs_position.py" GMSWA-v8drop15 "$RUN" 8192 120 \
    > "$ROOT/eval_results/ppl_GMSWA-v8drop15.out" 2>&1 || true
echo "[$(date)] NIAH sweep + recall (8 GPUs)" | tee -a "$LOG"
bash eval_suite.sh GMSWA-v8drop15=$RUN 2>&1 | tee -a "$LOG"
...
"$ROOT/.venv311/bin/accelerate" launch --num_processes 8 --num_machines 1 --main_process_port 29592 \
  ...
  --tasks "$SHORT" --batch_size 16 --output_path "$ROOT/eval_results/GMSWA-340M-v8drop15-10k/short" 2>&1 \
  | grep -avE "Generating|examples/s|Downloading|Noises" > "$ROOT/eval_results/GMSWA-340M-v8drop15-10k/short.log" || true
echo "[$(date)] V8DROP15 EVAL DONE" | tee -a "$LOG"
