...
cd "$FLAME"
LOG=$ROOT/eval_results/eval_v7drop.log
RUN=$FLAME/saves/GMSWA-340M-v7drop-10k
CFG=configs/gated_mem_swa_v7drop_340M.json
...
CUDA_VISIBLE_DEVICES=0 $PY "$ROOT/scripts/analysis/ppl_vs_position.py" GMSWA-v7drop "$RUN" 8192 120 \
    > "$ROOT/eval_results/ppl_GMSWA-v7drop.out" 2>&1 || true
echo "[$(date)] NIAH sweep + recall (8 GPUs)" | tee -a "$LOG"
bash eval_suite.sh GMSWA-v7drop=$RUN 2>&1 | tee -a "$LOG"
...
"$ROOT/.venv311/bin/accelerate" launch --num_processes 8 --num_machines 1 --main_process_port 29591 \
  ...
  --tasks "$SHORT" --batch_size 16 --output_path "$ROOT/eval_results/GMSWA-340M-v7drop-10k/short" 2>&1 \
  | grep -avE "Generating|examples/s|Downloading|Noises" > "$ROOT/eval_results/GMSWA-340M-v7drop-10k/short.log" || true
echo "[$(date)] V7DROP EVAL DONE" | tee -a "$LOG"
