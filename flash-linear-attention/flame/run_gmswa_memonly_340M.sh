...
"$VENV/bin/torchrun" \
  ...
  --rdzv_backend c10d --rdzv_endpoint "localhost:29536" \
  --local-ranks-filter 0 --role rank --tee 3 \
  --log-dir "$RUN/logs" \
  -m flame.train \
  --job.dump_folder "$RUN" \
  --model.config configs/gated_mem_swa_memonly_340M.json \
  ...
  --training.dataset /data/Minko/datasets/fineweb_edu_100BT \
