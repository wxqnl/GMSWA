...
from torchtitan.components.checkpoint import CheckpointManager, LR_SCHEDULER, OPTIMIZER
...
def main(job_config: JobConfig):
    ...
    try:
        ...
        if OPTIMIZER not in str(exc):
            logger.warning(
                "Checkpoint optimizer state appears incompatible with the current code. "
                "Retrying load without optimizer/lr_scheduler state."
            )
        checkpoint.exclude_from_loading = list(
            dict.fromkeys([*checkpoint.exclude_from_loading, OPTIMIZER, LR_SCHEDULER])
        )
        checkpoint.load(step=job_config.checkpoint.load_step)
