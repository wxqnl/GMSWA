...
def _detect_column_names(dataset: Union[Dataset, IterableDataset]) -> Optional[List[str]]:
    if dataset.column_names is not None:
        return dataset.column_names

    if isinstance(dataset, IterableDataset):
        first_sample = next(iter(dataset.take(1)), None)
...
def _select_text_column(
    dataset: Union[Dataset, IterableDataset],
    dataset_name: str,
) -> Union[Dataset, IterableDataset]:
    column_names = _detect_column_names(dataset)
    ...
    raise ValueError(
        f"Subset {dataset_name} has no 'text' or 'content' column"
        + (f"; detected columns: {column_names}" if column_names is not None else "")
    )
...
def build_dataset(
    ...
    if len(dataset.split(',')) == 1:
        ...
        if dataset_name is not None:
            dataset_names = [
                name.strip() or None for name in dataset_name.split(",")
            ]
        ...
        if dataset_split is not None:
            dataset_splits = [split.strip() or "train" for split in dataset_split.split(",")]
        ...
        if data_dir is not None:
            data_dirs = [
                data_dir.strip() or None for data_dir in data_dir.split(",")
            ]
        ...
        if data_probs is not None:
            data_probs = [float(p.strip()) for p in data_probs.split(",")]
