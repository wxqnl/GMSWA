...
set -uo pipefail
ROOT=/data/Minko/GMSWA; FLA=$ROOT/flash-linear-attention; FLAME=$FLA/flame
source "$ROOT/.venv311/bin/activate"
if [ -d /data/Minko/hf_offline ]; then
  export HF_HOME=${HF_HOME:-/data/Minko/hf_offline}
  export HF_HUB_OFFLINE=${HF_HUB_OFFLINE:-1}
  export HF_DATASETS_OFFLINE=${HF_DATASETS_OFFLINE:-1}
fi
if [ -d /data/Minko/nltk_data ]; then
  export NLTK_DATA=${NLTK_DATA:-/data/Minko/nltk_data}
fi
PY=$ROOT/.venv311/bin/python; export PYTHONPATH="$FLA:${PYTHONPATH:-}"; export HF_ALLOW_CODE_EVAL=1
cd "$FLAME"
LOG=$ROOT/eval_results/eval_memonly.log
RUN=$FLAME/saves/GMSWA-340M-memonly-10k
CFG=configs/gated_mem_swa_memonly_340M.json
...
$PY -m flame.utils.convert_dcp_to_hf --path "$RUN" --step 10000 \
    --config "$CFG" --tokenizer /data/Minko/models/gla-tokenizer 2>&1 | tail -3 | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=0 $PY "$ROOT/scripts/analysis/ppl_vs_position.py" GMSWA-memonly "$RUN" 8192 120 \
    > "$ROOT/eval_results/ppl_GMSWA-memonly.out" 2>&1 || true
echo "[$(date)] NIAH sweep + recall (8 GPUs)" | tee -a "$LOG"
bash eval_suite.sh GMSWA-memonly=$RUN 2>&1 | tee -a "$LOG"
...
"$ROOT/.venv311/bin/accelerate" launch --num_processes 8 --num_machines 1 --main_process_port 29594 \
  ...
  --tasks "$SHORT" --batch_size 16 --output_path "$ROOT/eval_results/GMSWA-340M-memonly-10k/short" 2>&1 \
  | grep -avE "Generating|examples/s|Downloading|Noises" > "$ROOT/eval_results/GMSWA-340M-memonly-10k/short.log" || true
echo "[$(date)] MEMONLY EVAL DONE" | tee -a "$LOG"
