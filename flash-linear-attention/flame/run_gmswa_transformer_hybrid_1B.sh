#!/usr/bin/bash
# 1B launch template for GMSWA-TransformerHybrid-1B-10k.
...
RUN=${RUN:-$FLAME/saves/GMSWA-TransformerHybrid-1B-10k}
...
"$VENV/bin/torchrun" \
  ...
  -m flame.train \
  --job.dump_folder "$RUN" \
  --model.config configs/gated_mem_swa_transformer_hybrid_1B.json \
