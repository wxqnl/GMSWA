...
class GatedMemSWA(nn.Module):
    ...
    def _project_memory_update(self, evicted_v: torch.Tensor) -> torch.Tensor:
        ...
        if self.mem_proj_mode == "scale":
            ...
            if evicted_v.dim() == 3:
                projected = evicted_v.unsqueeze(-2) * scale.unsqueeze(0)
                return self._normalize_memory(projected.to(torch.float32)).to(dtype=evicted_v.dtype)
            if evicted_v.dim() == 4:
                projected = evicted_v.unsqueeze(-2) * scale.view(
                    1,
                    1,
                    self.num_kv_heads,
                    self.num_mem_slots,
    ...
    def _load_cached_state(
        ...
        if past_key_values is None or self.layer_idx is None:
            return memory_state, k_cached, v_cached
    ...
    def _prepare_cache_state(
        ...
        memory_state, k_cached, v_cached = self._load_cached_state(
            past_key_values,
            batch_size,
            device,
            dtype,
    ...
    def _run_fused_memory_scan(
        ...
        *,
        ...
        if gates.shape[-1] == 1 and updates.shape[3] > 1:
            return self._run_shared_gate_memory_scan(
                gates,
                updates,
                memory_state,
        ...
        return self._run_per_slot_memory_scan(
            gates,
            updates,
            memory_state,
            output_final_state=output_final_state,
    ...
    def _run_shared_gate_memory_scan(
        ...
        *,
        ...
        flat_updates = updates.reshape(batch_size, seq_len, num_heads, self.num_mem_slots * self.head_dim).float()
        ...
        if gates.requires_grad or updates.requires_grad:
            if gates.is_cuda:
                ...
                def affine_prefix_scan(
                    ...
                    *,
                    ...
                    while offset < seq_len:
                        ...
                        if seq_ids is None:
                            prefix_gates = torch.cat(
                                [prev_gates[:, :offset], prev_gates[:, offset:] * prev_gates[:, :-offset]],
                                dim=1,
                            )
                    ...
                    init_tokens = init_state.unsqueeze(1) if seq_ids is None else init_state[seq_ids].unsqueeze(0)
                    return prefix_gates.unsqueeze(-1) * init_tokens + prefix_biases

                if cu_seqlens is None:
                    state_seq = affine_prefix_scan(flat_gates.float(), flat_updates, flat_state)
                    final_state = state_seq[:, -1] if output_final_state else None
                else:
                    seq_ids = prepare_sequence_ids(cu_seqlens)
                    state_seq = affine_prefix_scan(flat_gates.float(), flat_updates, flat_state, seq_ids=seq_ids)
                    final_state = None
            ...
            contrib = ((1.0 - flat_gates.float()).unsqueeze(-1) * flat_updates) / prefix.unsqueeze(-1)
    ...
    def _forward_cached(
        ...
        *,
        ...
        past_key_values.update(
            ...
            attn_state=(k.reshape(batch_size, 1, -1), v.reshape(batch_size, 1, -1)),
